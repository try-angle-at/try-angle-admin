    ...
    <v-container class="justify-center">
        ...
        <data-table
          :headers="[
            ...
            { text: '이름', value: 'name' },
            { text: '등록일', value: 'date' },
            { text: '액션', value: 'action', width: '150px' },
          ]"
          :items="tableItems"
          item-key="id"
          class="elevation-1"
...
const imageBaseUrl = (import.meta.env.VITE_IMAGE_BASE_URL || import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');
const tableItems = ref([
  { id: 1, image: `${imageBaseUrl}/reference_images/img1.png`, name: '참고 이미지 1', date: '2024-06-01' },
  { id: 2, image: `${imageBaseUrl}/reference_images/img2.png`, name: '참고 이미지 2', date: '2024-06-02' },
  { id: 3, image: `${imageBaseUrl}/reference_images/img3.png`, name: '참고 이미지 3', date: '2024-06-03' },
]);
...
function onSortChange({ key, order }) {
  // order: 'asc' | 'desc' | null
  // key: 정렬 대상 컬럼 키
  fetchList({ sortKey: key, sortOrder: order });
}

function fetchList({ sortKey, sortOrder }) {
  // TODO: 백엔드 API 연동 시 여기서 정렬 파라미터를 포함해 재조회
  console.log('[data-table] fetchList', { sortKey, sortOrder });
}
