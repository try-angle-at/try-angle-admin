...
function validateForm() {
  ...
  if (!name) {
    ...
    errorMsg.value.name = '태그명은 한글/영문/공백만 입력 가능합니다.';
  }
...
async function fetchTagDetail() {
  ...
  try {
    const response = await HttpHandler.getTag({ id: tagId });
    const statusCode = response?.status?.code;
    if (statusCode && statusCode !== 'S0000') {
      throw new Error(response?.status?.msg || '태그 상세 조회에 실패했습니다.');
    }

    const data = response?.data || {};
    form.value.parentCode = data.parentCode ?? null;
    form.value.code = data.code ?? '';
    form.value.name = data.tagName ?? '';
  } catch (error) {
    console.error('태그 상세 조회 실패:', error);
    openDialog(
      '조회 실패',
      error?.message || '태그 상세 정보를 불러오지 못했습니다.',
      () => {
...
async function updateTag() {
  ...
  try {
    const response = await HttpHandler.updateTag({
      id: tagId,
    ...
    const statusCode = response?.status?.code;
    if (statusCode && statusCode !== 'S0000') {
      throw new Error(response?.status?.msg || '태그 수정에 실패했습니다.');
    }
    ...
    openDialog(
      '수정 실패',
      error?.message || '태그 수정 중 오류가 발생했습니다.',
      () => {
        dialog.value.isActive = false;
      },
      true,
